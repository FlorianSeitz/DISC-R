rm(list = ls(all = TRUE))
pacman::p_load(data.table, R6, Rsolnp)
...
#   if (unidim == FALSE) {
#     m <- do.call(gcm, c(args, data = list(data[block == "training", ])))
#   } else {
#     args$fixed <- c(args$fixed, unlist(fitted_parm_gcm[metric == metr & subj_id == id, c("c", "tau")]))
#     print(args$fixed)
#     m <- do.call(gcm_unidim, c(args, data = list(data[!is.na(response) & block != "familiarization", ]), discount = sum(data$block == "training")))
#   }
...
# fit_ll <- function(data, metr, id = NULL, unidim = FALSE){
#   args <- list(formula = response ~ feature1 + feature2 + feature3, cat = ~ true_cat, metric = metr, fixed = c(r = 1, p = 1), choicerule = "soft")
#   last_trials <- data[, which(which(block == "training") > (sum(block == "training") - 100))]
#   if (unidim == FALSE) {
#     m <- do.call(gcm, c(args, data = list(data[block != "test"]), discount = list(last_trials)))
#   } else {
#     args$fixed <- c(args$fixed, unlist(fitted_parm_gcm[metric == metr & subj_id == id, c("c", "tau")]))
#     m <- do.call(gcm_unidim, c(args, data = list(data[!is.na(response) & trial %in% last_trials == FALSE, ]), discount = sum(data$block != "test")))
#   }
#   return(as.list(c(m$parm)))
# }
...
fit_ll <- function(data, metr, id = NULL, unidim = FALSE){
  args <- list(formula = response ~ feature1 + feature2 + feature3, cat = ~ true_cat, metric = metr, fixed = c(r = 1, p = 1), choicerule = "soft")
  if (unidim == FALSE) {
    m <- do.call(gcm, c(args, data = list(data[block != "test"])))
  } else {
    args$fixed <- c(args$fixed, unlist(fitted_parm_gcm[metric == metr & subj_id == id, c("c", "tau")]))
    m <- do.call(gcm_unidim, c(args, data = list(data[!is.na(response), ]), discount = sum(data$block != "test")))
  }
  return(as.list(c(m$parm)))
}

# Fit Parameters of discrete and Minkowski model 
fitted_parm_gcm <- rbindlist(lapply(c("disc", "mink"), function(x) dt[, {print(.GRP); fit_ll(.SD, metr = x, id = unique(subj_id))}, by = subj_id]), id = "metric")
fitted_parm_gcm[, metric := ifelse(metric == 1, "disc", "mink")]
fwrite(fitted_parm_gcm, file = "../../data/processed/categorization_main_fitted_parm_gcm_train3.csv")
...
fwrite(fitted_parm_gcm_unidim, file = "../../data/processed/categorization_main_fitted_parm_gcm_unidim_train3.csv")
