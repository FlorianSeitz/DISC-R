...
weights <- melt(weights, id.vars = c("subj_id", "time_pressure_cond", "ord"), variable.name = "model", value.name = "weight")
...
ggplot(weights, aes(x = ord, y = weight, fill = model)) +
  ...
  facet_wrap(time_pressure_cond~., ncol = 1, scales = "free", drop = TRUE, labeller = labeller(time_pressure_cond = labs)) +
  xlab("Participants") +
  ylab("Weights") +
