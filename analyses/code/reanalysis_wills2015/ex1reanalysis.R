...
for(i in 1:nrow(top)) {
  ...
  identity.attr <- Similarity(Stimuli = current.items, 
                              Metric = "Attr", 
                              c = 1, w = c(w1, w2), p = 1, r = 1)
  sim.identity.attr <- rbind(sim.identity.attr, identity.attr)
  
  identity.eucl <- Similarity(Stimuli = current.items, 
                              Metric = "Eucl", 
                              c = 1, w = c(w1, w2), p = 1, r = 1)
