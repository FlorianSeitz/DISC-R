...
rm(list = ls(all = TRUE))
pacman::p_load(gtools, RVAideMemoire, data.table, splitstackshape, esc, cognitiveutils)
set.seed(932)

# 0. Prepares data
setwd(dirname(rstudioapi::getActiveDocumentContext()$path)) # change this to the folder where this script is lying; works only in RStudio
dt <- fread("../../data/results/categorization_data_with_predictions_train2.csv", 
...
model_comparisons <- rbindlist(apply(model_pairs, 1, function(x) {
  ...
  model_comparison[, .(m1_better_m2 = ifelse(get(model_pair[1]) >= weights_crit,
                                             1,
                                             ifelse(get(model_pair[1]) <= (1 - weights_crit), -1, 0)),
                       m1 = model_pair[1],
...
weights_above_crit <- weights[, base::max(.SD) >= weights_crit, by = list(time_pressure_cond, subj_id)][, V1]
model_distr <- ll_test[weights_above_crit, .N, by = list(time_pressure_cond, best_fitting_model)]
...
ll_test[weights_above_crit, table(time_pressure_cond, best_fitting_model)]
...
weights_above_crit_stim <- weights_stim[, base::max(.SD) >= weights_crit, by = list(time_pressure_cond, subj_id, stim)][, V1]
ll_test_stim[weights_above_crit_stim, table(time_pressure_cond, best_fitting_model, stim)]
